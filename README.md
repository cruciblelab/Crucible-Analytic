...
every flow that can use email works without it. An invitation is a link
the installer copies; a password reset runs on recovery codes. Email adds
a second copy of a link somebody is already looking at, and never the
only copy.

The owner configures it at `/posta`. An admin cannot, and neither can a
developer session — whoever controls the outgoing mail server receives
every password-reset link the panel sends, so this is close enough to
"may become any user" that a redeemed developer link is refused before
ownership is even considered.

### What the page is actually for

SMTP fails in a handful of specific ways that all surface as
"authentication failed". The page names which one it was, and says what
to do about it:

| What the page says | What it means |
|---|---|
| The server could not be reached | Usually a blocked outbound port — most hosts block 25, some block 587 |
| The port and the encryption setting do not match | 587 wants STARTTLS, 465 wants TLS from the first byte. The page offers the other one |
| The certificate could not be verified | A self-signed mail server. There is no setting that turns verification off |
| No encryption was offered, so the password was not sent | Reported as itself, because "wrong password" would be false |
| The server only accepts OAuth | Basic SMTP auth is off for the tenant. No password will work; you need an app password |
| The sender address was refused | Providers usually only allow sending from the mailbox you authenticated as |

The connection test connects, encrypts and authenticates without sending
anything. A separate button sends a real message to an address you type.

### The password

It cannot be hashed: it has to be handed to somebody else's server on
every send. So it is encrypted instead, with `secret_key` from
`panel.toml` — 32 bytes, generated by `release/install.sh`, or by hand:

```
openssl rand -hex 32
```

What that protects against is precise: a copy of the database without the
configuration file. A nightly dump, a restore into a staging box, a
decommissioned replica. It does **not** protect against an attacker who
has the panel process or its filesystem — they have the key too.

With no `secret_key` the page still works and says so; an account with no
password (a local relay) can still be saved, and one with a password is
refused rather than stored in the clear.

The password never reaches the browser. The type the page renders has no
password field on it at all, which is checked by a test that reads the
field names out of the source.

### DKIM

Not checked, and the page says so rather than leaving it as an absence.
The panel sends mail and never receives any, so it cannot read the
headers of a message it sent; and whether a domain publishes a DKIM
record cannot be asked either, because the selector is chosen by the
provider. Every real provider signs already. SPF and DMARC *are* looked
up and reported — never enforced, because a record added a minute ago is
not visible yet, and a wizard that says "failed" about one teaches people
to click past the checks that are reliable.
